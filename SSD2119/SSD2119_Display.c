...
unsigned short textColor, backgroundColor;
...
static void LCD_GPIOInit(void){
    ...
    wait++;                                     // wait for port activation
    GPIO_PORTD_DIR_R |= 1 << 2;                   // make PD2 an output
    GPIO_PORTD_AFSEL_R &= ~1 << 2;                  // disable alternate functions
    ...
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTK_DIR_R |= 1 << 5;                  // make PK5 an output
    GPIO_PORTK_AFSEL_R &= ~1 << 5;                // disable alternate functions
    GPIO_PORTK_DEN_R |= 1 << 5;                      // enable digital I/O on PK5

    // M7
   SYSCTL_RCGCGPIO_R |= 1 << 11;             // activate port M
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
   GPIO_PORTM_DIR_R |= 1 << 7;                  // make PM7 an output
   GPIO_PORTM_AFSEL_R &= ~1 << 7;                 // disable alternate functions
   GPIO_PORTM_DEN_R |= 1 << 7;              // enable digital I/O on PM7

    // N4, N5
   SYSCTL_RCGCGPIO_R |= 1 << 12;               // activate port N
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTN_DIR_R |= (1 << 4) | (1<<5);               // make PN4, PN5 outputs
    GPIO_PORTN_AFSEL_R &= ~((1 << 4) | (1<<5));                // disable alternate functions
    GPIO_PORTN_DEN_R |= (1 << 4) | (1<<5);              // enable digital I/O on PN4, PN5

    // P0, P1, P3, P4
    SYSCTL_RCGCGPIO_R |= 1 << 13;                // activate port P
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTP_DIR_R |= (1 << 0) | (1<<1) | (1<<3) | (1<<4);            // make PP0-1, PP3-4 outputs
    GPIO_PORTP_AFSEL_R &= ~((1 << 0) | (1<<1) | (1<<3) | (1<<4));     // disable alternate functions
    GPIO_PORTP_DEN_R |= (1 << 0) | (1<<1) | (1<<3) | (1<<4);   // enable digital I/O on PP0-1, PP3-4

    // Q0, Q2, Q3
   SYSCTL_RCGCGPIO_R |= 1 << 14;                   // activate port Q
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
   GPIO_PORTQ_DIR_R |= (1 << 0) | (1<<2) | (1<<3);              // make PQ0, PQ2-3 outputs
   GPIO_PORTQ_AFSEL_R &= ~((1 << 0) | (1<<2) | (1<<3));       // disable alternate functions
   GPIO_PORTQ_DEN_R |= (1 << 0) | (1<<2) | (1<<3);   // enable digital I/O on PQ0, PQ2-3
}
...
void LCD_Init(void){
    ...
    // Set default text color to white on a black background
    LCD_SetTextColor(255, 255, 255);
    LCD_SetBackgroundColor(0, 0, 0);
}
...
void LCD_PrintChar(unsigned char data){
    ...
    // Or to the beginning of the current line
    else if (data == '\r') {
        LCD_SetCursor(0, cursorY);
    }
    ...
    for(i=0; i<5; i=i+1){
        ...
        for (j=0; j<8; j=j+1){
            ...
            LCD_DrawPixel(cursorX + i, cursorY + j, (tempData & 0x01) ? textColor : backgroundColor);
...
// - Sets the color that character backgrounds will be
//   printed in
// ********************************************************
void LCD_SetBackgroundColor(unsigned char r, unsigned char g, unsigned char b){
    backgroundColor = convertColor(r, g, b);
}
	...
	while (fmt[k] != 0) {
		if (fmt[k] == '%') {                    // Special escape, look for next arg
			if (fmt[k+1] == 'd') {              // Display integer
				...
				LCD_PrintString(x);
 			} else if (fmt[k+1] == 'f') {       // Display float (passed as double)
...
void LCD_PrintFloat(double num){
  ...
  double decimalPart = (num- integerPart);
    ...
    while (i < 3){
        ...
        onesPlace = (int)decimalPart;
        decimalPart = decimalPart - onesPlace ;
        tempString[i] = onesPlace + 48;
        i += 1;
    ...
    LCD_PrintInteger(integerPart);
    // Print out our string in reverse order
    for (int j = 0; j < 3; j++){
        LCD_PrintChar(tempString[j]);
    }
